...
class ProprioceptiveToddlerStep(CalibrationStep):
    ...
    def run(
        ...
        try:
            ...
            min_l = self._find_single_motor_breakaway(
                hw, recorder, motor_idx=0, label="Stage1_Left"
            )
            min_r = self._find_single_motor_breakaway(
                hw, recorder, motor_idx=1, label="Stage1_Right"
            )
            ...
            logger.info(
                f"[STAGE 1 RESULT] Left Breakaway={min_l:.1f}%, Right Breakaway={min_r:.1f}% -> min_power_visible={min_power:.1f}%"
            )
            ...
            if abs(min_l - min_r) > 25.0:
                logger.error(
                    f"[FATAL] Severe asymmetry between motors (|{min_l:.1f}% - {min_r:.1f}%| > 25%)."
                )
                return StepStatus.FATAL, {}, {}
            ...
            axes_config, swap_lr = self._derive_kinematics_and_polarity(
                hw, recorder, test_pwm, label="Stage2_Kinematics"
            )
            if not axes_config:
            ...
            flip_pwm, rest_b = self._run_symmetric_rock_and_check(
                hw,
                recorder,
                start_pwm=max(15.0, min_power + 10.0),
                step=5.0,
                label="Stage3_RockAndCheck",
            )
            if flip_pwm is None:
    ...
    def _sample_settled_pitch(
        self, hw: RobotHardware, recorder: DiscoveryTelemetryRecorder, stage: str
    ) -> float:
        samples = []
        for _ in range(10):
            if hw.watchdog:
    ...
    def _find_single_motor_breakaway(
        self, hw: RobotHardware, recorder: DiscoveryTelemetryRecorder, motor_idx: int, label: str
    ) -> float | None:
        ...
        for pwm in range(0, 61, 1):
            ...
            if max_gyro > 3.0:
                logger.info(
                    f"  [{label}] Motion detected at {pwm}.0% PWM (max_gyro={max_gyro:.1f} deg/s)"
                )
                return float(pwm)

        return None

    def _derive_kinematics_and_polarity(
        self, hw: RobotHardware, recorder: DiscoveryTelemetryRecorder, test_pwm: float, label: str
        ...
        vert_axis = max(vert_candidates, key=lambda k: vert_candidates[k])
        ...
        l_gyro, l_accel = self._pulse_and_measure_with_home(
            hw, recorder, test_pwm, test_pwm, f"{label}_Straight"
        )
        # 3. Pulse spin (test_pwm, -test_pwm), then return to home (-test_pwm, test_pwm)
        s_gyro, _s_accel = self._pulse_and_measure_with_home(
            hw, recorder, test_pwm, -test_pwm, f"{label}_Spin"
        )
        ...
        if yaw_straight > yaw_spin:
            logger.info(
                f"  [{label}] Polarity Mismatch Confirmed (straight yaw={yaw_straight:.1f} > spin yaw={yaw_spin:.1f}). Inverting Right Motor."
            )
            motor_r_invert = True

        # Pitch Axis: dominant gyro orthogonal to Vertical
        pitch_candidates = {k: abs(getattr(l_gyro, k)) for k in axes if k != vert_axis}
        pitch_axis = max(pitch_candidates, key=lambda k: pitch_candidates[k])
        ...
        fwd_axis = max(fwd_candidates, key=lambda k: fwd_candidates[k])
    ...
    def _pulse_and_measure(
        self,
        hw: RobotHardware,
        recorder: DiscoveryTelemetryRecorder,
        cmd_l: float,
        cmd_r: float,
        label: str,
    ) -> tuple[Any, Any]:
    ...
    def _pulse_and_measure_with_home(
        self,
        hw: RobotHardware,
        recorder: DiscoveryTelemetryRecorder,
        cmd_l: float,
    ...
    def _run_symmetric_rock_and_check(
        self,
        hw: RobotHardware,
        recorder: DiscoveryTelemetryRecorder,
        start_pwm: float,
        ...
        while p <= 60.0 and attempts < 4:
            ...
            if hw.watchdog:
                hw.watchdog.heartbeat()
            logger.info(
                f"  [Symmetric Search] Testing rock-and-check at {p:.1f}% PWM (attempt {attempts}/4)..."
            )
            # 1. Try Direction +1.0
            flipped, settled_pitch = self._attempt_rock(
                hw, recorder, 1.0, p, f"{label}_DirPlus_{p}"
            )
            if flipped:
                logger.info(
                    f"  [FOUND] KickUp success in Direction +1.0 at {p:.1f}% PWM (settled={settled_pitch:.1f}°)"
                )
                return p, settled_pitch

            # 2. Try Direction -1.0
            flipped, settled_pitch = self._attempt_rock(
                hw, recorder, -1.0, p, f"{label}_DirMinus_{p}"
            )
            if flipped:
                logger.info(
                    f"  [FOUND] KickUp success in Direction -1.0 at {p:.1f}% PWM (settled={settled_pitch:.1f}°)"

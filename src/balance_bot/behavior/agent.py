...
class Agent:
    ...
    def __init__(self, watchdog: Optional[SurvivalWatchdog] = None):
        ...
        self.has_saved_state = LEARNING_STATE_FILE.exists()
        force_calib = check_force_calibration_flag()

        # Load Hardware Config (Immutable)
        self.hw_config = HardwareConfig.load()

        if force_calib:
            logger.info("Forcing calibration: Using default learning state.")
            ...
            if self.has_saved_state:
                ...
                logger.info(">>> No Learning State Found. Starting in FIRST RUN MODE.")
                self.first_run = True
            ...
            while self.running:
                ...
                if self.ticks % 10 == 0:
                    ...
                    if self.config_dirty and (time.monotonic() - self.last_save_time > self.hw_config.timing.save_interval):
                        try:
                            # Use model_dump for Pydantic V2 compat (or dict() for V1)

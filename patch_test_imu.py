...
# Modify the test that expected stale data to now expect an exception
replacement = """    def test_imu_resilience_zero_order_hold(monkeypatch):
        \"\"\"
...
content = re.sub(r'    def test_imu_resilience_zero_order_hold\(monkeypatch\):.*?# This calls read_imu_raw which should catch the error\n        accel, gyro = hw\.read_imu_raw\(\)\n\n        assert accel == glm\.vec3\(0\)\n        assert gyro == glm\.vec3\(0\)', replacement, content, flags=re.DOTALL)

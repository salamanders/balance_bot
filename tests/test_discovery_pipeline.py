...
def test_pipeline_flow() -> None:
    ...
    with (
        patch("balance_bot.discovery.pipeline.RobotHardware", mock_hw_cls),
...
def test_pipeline_needs_retry() -> None:
    ...
    with (
        ...
        step.run.side_effect = [(StepStatus.NEEDS_RETRY, {}, {}), (StepStatus.SUCCESS, {}, {})]
...
def test_pipeline_unexpected_error() -> None:
    ...
    with (
        ...
        with pytest.raises(
            RuntimeError, match="Pipeline failed at ErrorStep: Some unexpected error"
        ):
            pipeline.run()

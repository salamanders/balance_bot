...
def test_calibrate_orientation(wc_fixture):
    ...
    with patch("builtins.input") as mock_input:
        # We need to configure the existing mock instance 'wc.hw'
        # because calibrate_static_orientation uses self.hw directly.
        wc.hw.read_imu_converted.return_value.pitch_angle = 30.0
        wc.hw.wait_for_stability = MagicMock()

        # The function uses self.hw.wait_for_stability

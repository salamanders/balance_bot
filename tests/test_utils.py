...
def test_get_i2c_failure_report_no_bus(mock_exists: Any) -> None:
    ...
    assert (
        result
        == "CRITICAL FAILURE: I2C Bus 1 (/dev/i2c-1) does not exist. The kernel driver is not loaded. Enable I2C in raspi-config or /boot/config.txt."
    )
...
def test_get_i2c_failure_report_no_permission(
    mock_env_get: Any, mock_access: Any, mock_exists: Any
) -> None:
    mock_exists.return_value = True
    ...
    assert (
        result
        == "CRITICAL FAILURE: Permission denied accessing /dev/i2c-1. User 'testuser' cannot read/write. Run: sudo usermod -aG i2c $USER"
    )
...
def test_get_i2c_failure_report_confusion(
    mock_run: Any, mock_access: Any, mock_exists: Any
) -> None:
    mock_exists.return_value = True
    ...
    assert (
        result
        == "CONFUSION: Device MPU6050 (0x68) IS detected on Bus 1 by i2cdetect, but Python driver failed. Check for library version mismatch or intermittent wiring connection."
    )
...
@patch("os.access")
@patch("subprocess.run")
def test_get_i2c_failure_report_not_found(
    mock_run: Any, mock_access: Any, mock_exists: Any
) -> None:
    mock_exists.return_value = True
    mock_access.return_value = True
    ...
    assert (
        result
        == "Hardware not found or failed.  You may be using a different bus, try 'Check I2C Bus'.  Also check wire connections."
    )
...
@patch("os.access")
@patch("subprocess.run")
def test_get_i2c_failure_report_missing_i2cdetect(
    mock_run: Any, mock_access: Any, mock_exists: Any
) -> None:
    mock_exists.return_value = True
    mock_access.return_value = True
...
@patch("os.access")
@patch("subprocess.run")
def test_get_i2c_failure_report_unknown_error(
    mock_run: Any, mock_access: Any, mock_exists: Any
) -> None:
    mock_exists.return_value = True
    mock_access.return_value = True

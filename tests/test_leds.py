...
def test_signal_ready():
    with patch.object(Path, "exists", return_value=False):
        controller = LedController()
        controller.set_led = MagicMock()

        controller.signal_ready()
        assert controller.mode == "ON"
        controller.set_led.assert_called_once_with(True)
...
def test_signal_off():
    with patch.object(Path, "exists", return_value=False):
        ...
        controller.set_led.assert_called_once_with(False)
...
def test_update_blinking():
    with patch.object(Path, "exists", return_value=False):
        ...
        with patch("time.monotonic", return_value=100.05):
            controller.update()
            controller.set_led.assert_not_called()
...
def test_blink():
    with patch.object(Path, "exists", return_value=False):
        ...
        with patch("time.sleep") as mock_sleep:
            ...
            assert controller.set_led.call_args_list == [
                call(True),
                call(False),
                call(True),
                call(False),
...
def test_countdown():
    with patch.object(Path, "exists", return_value=False):
        ...
        controller = LedController(config)
        controller._blink = MagicMock()

        with patch("time.sleep") as mock_sleep:
            controller.countdown()

            assert controller._blink.call_args_list == [
                call(3, 0.1, 0.1),
                call(2, 0.1, 0.1),
                call(1, 0.1, 0.1)
            ]

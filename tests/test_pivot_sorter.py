import math
import pytest
from balance_bot.utils import Vector3, sort_resting_vectors, vector_angle
...
def test_sort_resting_vectors_two_groups():
    ...
    # Pivot will be (1, 0, 0)
    v1 = Vector3(1.0, 0.0, 0.0)
    v2 = Vector3(0.98, 0.02, 0.0) # Very close to v1 (angle < 20)

    v3 = Vector3(0.0, 1.0, 0.0) # 90 deg away
    v4 = Vector3(0.02, 0.98, 0.0) # Very close to v3
    ...
    # Check return type
    assert isinstance(avg_a, Vector3)
    assert isinstance(avg_b, Vector3)

def test_sort_resting_vectors_single_cluster_raises():
    # Only vectors around (1, 0, 0)
    v1 = Vector3(1.0, 0.0, 0.0)
    v2 = Vector3(0.99, 0.01, 0.0)
    v3 = Vector3(1.0, 0.05, 0.0)

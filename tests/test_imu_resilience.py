...
def test_imu_resilience_zero_order_hold(mock_hardware_factory):
    """
    ...
    """
    hw = mock_hardware_factory(motor_l=0, motor_r=1)
    ...
    hw.sensor.get_accel_data.side_effect = OSError("Input/output error")
    # Note: get_gyro_data will be called if get_accel_data succeeds, or if get_accel_data fails?
    # read_imu_raw calls get_accel_data then get_gyro_data.
    # If get_accel_data fails, it raises OSError immediately.
    # So get_gyro_data is not called.
    # However, let's make sure get_gyro_data also would fail to simulate full bus failure
    hw.sensor.get_gyro_data.side_effect = OSError("Input/output error")
